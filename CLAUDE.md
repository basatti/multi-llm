...
# Project-specific rules — LLM Platform

These exist because the platform spans four layers with strict isolation, real secrets, real money (GPU spend), and (in cloud deployments) a public TLS endpoint that real applications integrate against. Breaking any of these costs hours or burns money — don't.

## 5. Layer isolation is the whole point

Read `docs/architecture.md` §3 before touching infrastructure. The contract:

- **Applications → gateway** only for stateless one-shot completions
- **Applications → orchestration** for anything stateful (sessions, memory, tool loops, app registry)
- **Orchestration → gateway** for every model call (with a virtual key, never provider creds)
- **Gateway → inference** only. Inference layer is *stateless* — no DB, no sessions, no tool calls
- **No layer reaches across** — orchestration never talks to the inference backend directly, applications never see physical model names, the gateway never knows about conversations

If you find yourself adding state to the gateway, or session-awareness to inference, stop. You're breaking the invariant the platform exists to protect.
...
1. **Never in the repo.** Not in YAML, not in Dockerfiles, not in compose, not in terraform `.tfvars`. The only place a value lives is your secret manager (AWS Secrets Manager / SSM Parameter Store for the cloud reference, Vault / sealed K8s secrets / whatever your platform uses for other deployments). Locally, `.env` is `.gitignore`'d.
...
- **Region** for the reference deployment is `ap-south-1` (Mumbai). Operators in other regions should set the relevant `*.tfvars` and check GPU quota first — the G/VT vCPU quota is per-region and per-account, defaults to 0 on new accounts, and takes hours to days for a first increase.
- **TLS** in the reference deployment uses a wildcard ACM cert that the operator owns. Don't generate new certs unless you need a different SAN; supply your own cert ARN via `*.tfvars`.
...
- **EC2 access** is via SSM Session Manager, not SSH. No SSH key is issued. To reach a non-public port (Langfuse, orchestration, the inference backend), use `aws ssm start-session --document-name AWS-StartPortForwardingSession`.

## 12. Long-running, model-loading services — be patient and verify, don't poll naively

First-boot timing on a single-GPU box (e.g. g4dn.xlarge / equivalent on-prem):
- Docker image pulls: 3–8 min (Langfuse stack + Ollama)
- Ollama model downloads (three default models): 8–15 min (~25 GB total)
- First model load to GPU on first request: 30–90 s
- Langfuse migration: 1–2 min
- App registration: <30 s

Total: **10–20 minutes on first boot**, ~1 minute on subsequent restarts (model cache survives the volume). If your script polls and gives up after 5 minutes, you'll declare failure when nothing is actually broken. Set timeouts accordingly.

The cloud bootstrap log on the instance is `/var/log/llm-bootstrap.log` — start there for any diagnosis. `docker ps` next. `docker logs <container>` last. For self-hosted, `docker compose logs -f` is the canonical source.
...
# AWS reference deployment — pull the issued API keys (substitute your region + env)
aws ssm get-parameters-by-path --region <region> \
  --path /llm-platform/<env>/apps --recursive --with-decryption \
  --query 'Parameters[].[Name,Value]' --output table

# AWS reference deployment — reach the instance shell
aws ssm start-session --region <region> --target <instance-id>

# Self-hosted — tail every service
